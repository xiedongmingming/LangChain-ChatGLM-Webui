...
if __name__ == "__main__":
    ...
    with block as demo:
        ...
        目前项目提供基于<a href='https://github.com/THUDM/ChatGLM-6B' target="_blank">CAHTGLM-6B</a>的LLM和包括GANYMEDENIL/TEXT2VEC-LARGE-CHINESE、NGHUYONG/ERNIE-3.0-BASE-ZH、NGHUYONG/ERNIE-3.0-NANO-ZH在内的多个EMBEDDING模型, 支持上传TXT、DOCX、MD、PDF等文本格式文件。<br>
        ...
        with gr.Row():
            ...
            send.click(
                predict,
                inputs=[
                    message, file, use_web, top_k, history_len, temperature, top_p, state
                ],
            ...
            clear_history.click(
                fn=clear_session,
                inputs=[],
                outputs=[chatbot, state],  # CHATBOT置空、STATE为NONE
                queue=False
            )

            message.submit(  # 同发送逻辑一致
                predict,
                inputs=[
                    message, file, use_web, top_k, history_len, temperature, top_p, state
                ],
    ...
    demo.queue(
        concurrency_count=3  # threads to consume the request
    ).launch(
        server_name='0.0.0.0',  # ip for listening, 0.0.0.0 for every inbound traffic, 127.0.0.1 for local inbound
